...
export const procesamientoImagenes = async ({
  archivos,
  imagenesDB,
  slotsParseados,
  id_producto,
    ...
    for (let i = 0; i < slotsParseados.length; i++) {
      ...
      if (slot === "NUEVA") {
        ...
        if (imagenAnterior && imagenAnterior.nombre_archivo) {
          try {
            const nombreArchivoViejo = imagenAnterior.nombre_archivo
              .split("/")
              .pop();

            if (nombreArchivoViejo) {
              const blockBlobClienteViejo =
                containerProductos.getBlockBlobClient(nombreArchivoViejo);
        ...
        if (archivo) {
          ...
          const blockBlobCliente =
            containerProductos.getBlockBlobClient(nombreArchivo);
