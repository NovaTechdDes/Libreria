...
import { Server, Network, ArrowRight, Loader2, BookOpen, AlertCircle, Sun, Moon } from 'lucide-react';
...
export const ServerSetup = ({ onConfigured }: { onConfigured: () => void }) => {
  ...
  useEffect(() => {
    const current = getServerUrl() || localStorage.getItem('server_url');
    if (current) {
      setUrl(current);
    }
  }, []);
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] text-slate-500 flex items-center gap-1">
                  <Network className="w-3 h-3 text-amber-500/80" /> IP / Host
                </span>
              </div>
