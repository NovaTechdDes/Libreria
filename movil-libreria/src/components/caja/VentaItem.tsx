...
export default function VentaItem({ venta }: Props) {
  ...
  const getTipoStyles = (tipo: string) => {
    switch (tipo) {
      ...
      case "Crédito":
        return {
          container: isDark
            ? "bg-violet-900/20 border-violet-900/30"
  ...
  const styles = getTipoStyles(venta?.tipo_venta || "Efectivo");
    ...
    <View className="flex-row items-center bg-white dark:bg-slate-900 py-4 px-1">
      ...
      <View className="flex-1 justify-center">
        <View className="flex-row items-center mb-1">
          <Text
            className="text-gray-800 dark:text-slate-100 font-bold text-base mr-2"
            numberOfLines={1}
          >
