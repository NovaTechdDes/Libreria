...
const BtnCarrito = ({ onClick }: { onClick: () => void }) => (
  <button
    onClick={onClick}
    className="w-full flex items-center justify-center gap-2 bg-primary hover:brightness-95 active:scale-[0.97] text-white text-[13px] font-semibold py-2.5 rounded-lg transition-all duration-150"
  >
  ...
  return (
    <article className="group flex flex-col bg-white rounded-xl overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.06),0_4px_16px_rgba(0,0,0,0.06)] hover:shadow-[0_4px_8px_rgba(0,0,0,0.08),0_12px_32px_rgba(0,0,0,0.10)] hover:-translate-y-1 transition-all duration-[180ms] ease-in-out cursor-pointer">
      ...
      <div className="flex flex-col gap-1 flex-1 px-4 pt-3.5 pb-4">
        {/* Rubro / categoría */}
        <p className="text-[10px] font-semibold tracking-[0.08em] uppercase text-primary m-0">
          {producto.id_rubro ? `Rubro ${producto.id_rubro}` : "Sin categoría"}
        </p>

        {/* Nombre del producto */}
        <h3 className="text-[15px] font-bold text-[#1a1a18] mt-0.5 leading-[1.3] line-clamp-2">
          ...
          {producto.cantidad !== undefined && (
            <span
              className={`text-[10px] font-semibold px-2 py-0.5 rounded-full tracking-[0.04em] ${
                producto.cantidad > 0
                  ? "bg-[rgba(107,143,107,0.12)] text-[#4a7c4a]"
                  : "bg-[rgba(180,60,60,0.10)] text-[#b43c3c]"
              }`}
            >
              {producto.cantidad > 0
