  ...
  return (
    <aside className="w-[240px] h-screen bg-white border-r border-slate-400 flex flex-col py-8 shadow-[1px_0_10px_rgba(0,0,0,0.02)]">
      ...
      <div className="flex flex-col items-center gap-2 px-4">
        ...
        <h2 className="text-center text-2xl font-bold text-slate-900 mb-8">Lachi Admin</h2>
      </div>
      <nav className="flex-1 px-4 space-y-2">
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              <Icon className={`w-5 h-5 ${isActive ? 'text-teal-500' : 'text-slate-400 group-hover:text-slate-900'}`} />
              <span className="text-[15px]">{item.label}</span>

              {/* Indicador lateral activo */}
              {isActive && <div className="absolute right-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-teal-500 rounded-l-full" />}
            </Link>
          );
        })}
      </nav>
      ...
      {/* Botón Cerrar Sesión */}
      <div className="px-4 mt-auto border-t border-slate-50">
        <button onClick={handleLogOut} className="w-full flex items-center gap-3 px-4 py-4 text-slate-500 hover:text-red-500 font-bold transition-all group">
