  ...
  return (
    <div className="bg-white rounded-[24px] border border-slate-200 shadow-sm p-8 w-full lg:sticky lg:top-8 animate-in fade-in slide-in-from-right-4 duration-500">
      ...
      <div className="space-y-6">
        ...
        <div>
          <label className="block text-[14px] font-bold text-slate-400 uppercase tracking-wide mb-2" htmlFor="nombre">Descripción</label>
        ...
        <div>
          ...
          <div className="flex gap-3">
            ...
            <div className="w-[85px] h-[85px] border-2 border-dashed border-slate-200 rounded-2xl flex flex-col items-center justify-center bg-slate-50 group cursor-pointer hover:border-teal-500/50 hover:bg-teal-50/30 transition-all">
              ...
              <span className="text-[10px] font-black text-slate-400 group-hover:text-teal-500 uppercase tracking-tighter">
                Principal
              </span>
            </div>
            {/* Secondary 1 */}
        ...
        <div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            {productoSeleccionado.colores?.map((color, i) => (
              <div
                key={i}
                className={`w-10 h-10 rounded-full border-2 border-white shadow-sm cursor-pointer hover:scale-110 transition-transform ${i === 0 ? "ring-2 ring-teal-500 ring-offset-2" : ""}`}
                style={{ backgroundColor: color.color }}
              />
            ))}
            <button className="w-10 h-10 rounded-full border-2 border-dashed border-slate-200 flex items-center justify-center text-slate-400 hover:border-teal-500 hover:text-teal-500 transition-all">
              <FiPlus className="w-5 h-5" />
            </button>
          </div>
          <p className="text-[10px] font-black text-slate-400 mt-3 tracking-widest uppercase">
            Click para agregar nuevo tono
          </p>
        </div>

        {/* Submit */}
        <button className="w-full bg-[#0096B1] text-white rounded-[20px] py-4.5 px-6 font-bold flex items-center justify-center gap-3 shadow-lg shadow-teal-100 hover:bg-[#008199] transition-all mt-4 group">
          <FiUploadCloud className="w-6 h-6 group-hover:-translate-y-1 transition-transform" />
          <span className="text-[17px]">Publicar Producto</span>
        </button>
